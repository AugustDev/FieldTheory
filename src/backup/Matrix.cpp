#include "Matrix.h"
    ...
    {
        assert(row + side < num_rows);
        assert(column + side < num_columns);
    ...
    void Matrix::InsertPotentialWireHorizontal(int row, int column, int length, double Potential)
    {
        ...
        assert(column + length <= num_columns);
    ...
    void Matrix::InsertPotentialCircular(int row_c, int column_c, int radius, double Potential)
    {
        ...
        */
        ...
        assert(column_c >= 0 && row_c >= 0);
        assert(column_c + radius < num_columns);
        assert(column_c + radius < num_rows);
